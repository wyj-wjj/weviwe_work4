...
- **文档解析导入防压缩与突破拆解上限**
  - **问题**：长篇问答对导入为“核心基础话术（`base_script`）”时正文被严重压缩，且标准化话术最多只能拆分 20 个子块。
  - **原因**：大模型 Prompt 对 `base_script` 下达了不可拆分+概括整理的冲突指令；后端逻辑硬编码了 `[:20]` 的建议切片。
  - **解决**：修改 DashScope Prompt，对所有类型加入“绝对禁止删减、缩写或概括提炼正文”的强制约束；修改 `document_structuring_service.py` 允许 `base_script` 拆分，并移除所有 `[:20]` 的硬限制。
  - **状态**：✅ 已完成，代码仅在本地提交，后端测试通过（`test_content_import_phase12.py`）。
